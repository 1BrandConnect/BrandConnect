...
//            <a class="crb-button-one" style="background-color: #3B5998;" href="https://www.facebook.com/wpcerber/" target="_blank"><span class="dashicons dashicons-facebook"></span> Follow Cerber on Facebook</a>
    ...
    */
	...
	$aside[] = '<div class="crb-box" id = "crb-blog">
            ...
            <p><a href="https://wpcerber.com/getting-started/" target="_blank">Getting Started Guide</a>
            <p><a href="https://wpcerber.com/wordpress-application-passwords-how-to/" target="_blank"> Managing WordPress application passwords a hassle-free way</a>
			...
			<p><a href="https://wpcerber.com/limiting-concurrent-user-sessions-in-wordpress/" target="_blank">How to stop your users from sharing their WordPress accounts</a>
...
function cerber_show_admin_notice(){
	...
	if ( $all ) {
		...
		' . $notice[0] . '<button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button></div>';
		}
	}
	...
	if ( $rows ) {
		...
		foreach ($rows as $row) {
			...
			if ( ! empty( $row->php_errors ) ) {
			    ...
			    if ( $row->php_errors[0] == 'a' ) { // OLD
					$err_list = @unserialize( $row->php_errors );
				}
				else {
					$err_list = @json_decode( $row->php_errors, true );
				}
