...
function dmidp_active_cron_job_after_five_second() {
	...
	$notices = file_get_contents('http://api.myrecorp.com/dmidp_notices.php?url='.$home_url);
...
function dmidp_right_side_notice(){
	...
	if (!empty($notices)) {
		foreach ($notices as $key => $notice) {
			...
			if ( $status && $is_right_sidebar == 1 && $current_time > $publish_time && $current_time < $auto_hide_time  ) {
...
function dmidp_admin_notices(){
	...
	if (!empty($notices)) {
		foreach ($notices as $key2 => $notice) {
			...
			if ( $status && !$is_right_sidebar && $current_time > $publish_time && $current_time < $auto_hide_time && !in_array($key, $clicked_data) ) {
				$html .=  '<div class="notice notice-'. $alert_type .' is-dismissible dcim-alert" dmidp_notice_key="'.$key.'">
						'.$content.'
					</div>';
			}
		}
